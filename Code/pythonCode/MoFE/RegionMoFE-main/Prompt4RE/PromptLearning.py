...
def crimePred(counts, text_embs, region_embs, kf_splits=10, epochs=1000, lr=1e-3, text_output_dim=args.text_output_dim, device="cuda"):
    ...
    for train_index, test_index in kf.split(index):
        ...
        best_r2 = float("-inf")
        for i in tqdm(range(epochs), desc=f"Fold {kf_idx} Prompt Learning"):
            optimizer.zero_grad()
...
def run_task(task="crime", text_embs=None, region_embs=None, device="cuda",
             kf_splits=10, epochs=args.epochs, lr=args.lr, text_output_dim=args.text_output_dim):
    if task == "checkIn":
        labels = np.load(f"../data_{args.city}/check_counts.npy")
        mask = labels > 0
        labels, text_embs, region_embs = labels[mask], text_embs[mask], region_embs[mask]
    elif task == "crime":
        labels = np.load(f"../data_{args.city}/crime_counts.npy")
    elif task == "serviceCall":
        labels = np.load(f"../data_{args.city}/serviceCall_counts.npy")
    elif task == "pop":
        labels = np.load(f"../data_{args.city}/population_counts.npy")
    else:
        raise ValueError("Unknown task!")
...
if __name__ == '__main__':
    ...
    print(model_params)
    region_emb_path = f"../best_emb_{task}.npy"
    # region_emb_path = f"./mmdata_{args.city}/best_emb_{task}.npy"
    prompt_emb_path = f"./mmdata_{args.city}/prompt_embs_{args.model}_{task}.npy"
    region_embs = np.load(region_emb_path)   
    prompt_embs = np.load(prompt_emb_path)  
